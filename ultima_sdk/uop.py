...
def create_hash(s: str) -> int:
    ...
    if len(b) - i > 0:
        remain = len(b) - i
        if remain >= 12: esi = _u32(esi + (b[i + 11] << 24))
        if remain >= 11: esi = _u32(esi + (b[i + 10] << 16))
        if remain >= 10: esi = _u32(esi + (b[i + 9]  << 8))
        if remain >=  9: esi = _u32(esi + b[i + 8])
        if remain >=  8: edi = _u32(edi + (b[i + 7] << 24))
        if remain >=  7: edi = _u32(edi + (b[i + 6] << 16))
        if remain >=  6: edi = _u32(edi + (b[i + 5] << 8))
        if remain >=  5: edi = _u32(edi + b[i + 4])
        if remain >=  4: ebx = _u32(ebx + (b[i + 3] << 24))
        if remain >=  3: ebx = _u32(ebx + (b[i + 2] << 16))
        if remain >=  2: ebx = _u32(ebx + (b[i + 1] << 8))
        if remain >=  1: ebx = _u32(ebx + b[i])
